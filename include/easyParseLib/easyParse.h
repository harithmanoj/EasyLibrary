...
namespace easyParse
{
    ...
    {
        return easyMath::valueBetweenInclusive(digit, '0', '9') 
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} stringConv.cpp binaryStringExample
     * 
     * Console Output:
     * \include{lineno} binaryString.out.txt
     */
    ...
    [[nodiscard]] constexpr inline Integer convertBinaryString(std::string_view numberString) noexcept
    {
        ...
        auto size = std::min(easyMath::bitSize<Integer>(), numberString.size());
    ...
    namespace impl_detail_
    {
        ...
        [[nodiscard]] constexpr inline Integer convertDirectTranslatableBaseString_(
        ...
        ) noexcept
        {
            ...
            {
                ret = ret << perBitSize;
                ret += convertDigit_<perBitSize>(digit);
            }
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} stringConv.cpp hexStringExample
     * 
     * Console Output:
     * \include{lineno} hexString.out.txt
     */
    template<easyMath::UnsignedIntegral Integer>
    ...
    {
        return impl_detail_::convertDirectTranslatableBaseString_<Integer, 4>(numberString);
    }
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} stringConv.cpp octStringExample
     * 
     * Console Output:
     * \include{lineno} octString.out.txt
     */
    template<easyMath::UnsignedIntegral Integer>
    ...
    {
        return impl_detail_::convertDirectTranslatableBaseString_<Integer, 3>(numberString);
    }
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} stringConv.cpp decStringExample
     * 
     * Console Output:
     * \include{lineno} decString.out.txt
     */
    template<easyMath::UnsignedIntegral Integer>
    ...
    /***
     * String where each character is a whitespace character.
     * 
     * contains: '\n', '\t', '\r', '\0'.
     */
    ...
    /**
     * @brief Extract string till a delimiter, modify initial string to remove the extracted string and delimiter.
     * 
     * converts `"hello\nall"` with delim '\n' to `"all"` and returns `"hello"`
     * 
    ...
    /**
     * @brief Check if string is a valid hexadecimal number string. Returns true if valid.
     * 
     * Allows: '0' - '9', 'a' - 'f', 'A' - 'F'.
     * 
    ...
    /**
     * @brief Check if string is a valid octal number string. Returns true if valid.
     * 
     * Allows: '0' - '7'.
     * 
     * @param[in] numberString string to validate.
    ...
    /**
     * @brief Check if string is a valid decimal number string. Returns true if valid.
     * 
     * Allows: '0' - '9'.
     * 
     * @param[in] numberString string to validate.
    ...
    /**
     * @brief Check if string is a valid binary number string. Returns true if valid.
     * 
     * Allows: '0', '1'.
     * 
     * @param[in] numberString string to validate.
     ...
     * 
     * Advances `iterator` and returns a char along with an indiction if character was escaped.
     * 
     * @throw (1) `std::invalid_argument`: If `iterator >= sentinel`.
     ...
     * @param[in] sentinel iterator to one over last character.
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} advance.cpp advanceOverTextExample
     * 
     * Console Output:
     * \include{lineno} advanceOverText.out.txt
     */
     ...
     * @param[inout] sentinel sentinel to guard against out of range access.
    ...
    )
    {
        ...
        eval = advanceOverText(iterator, sentinel);
        ...
        {
            while ((iterator < sentinel) && (isText))
            {
                eval = advanceOverText(iterator, sentinel);
    ...
    {
        return advanceSkipReportQuotedText(iterator, sentinel).first;
    }
     ...
     * <b>Example </b>
     * 
     * \snippet{lineno} stringConv.cpp numStringExample
     * 
     * Console Output:
     * \include{lineno} numString.out.txt
     */
    template<easyMath::UnsignedIntegral UInteger>
    [[nodiscard]] constexpr inline UInteger convertNumberString(std::string_view numberString)
    {
     ...
     */
    [[nodiscard]] constexpr inline bool validateNumberString(std::string_view numberString)
    {
        if(numberString.size() == 0)
            throw std::invalid_argument("empty string");
    ...
    /**
     * @brief Check whether the root line at offset has a substring matching `check`.
     * 
    ...
    /**
     * @brief Convert argument string to lower case.
     * 
     * @param[inout] str string to convert.
     */
    constexpr inline void toLower(std::string& str)
    {
        for(auto& ch : str)
        {
            if(easyMath::valueBetweenInclusive(ch, 'A', 'Z'))
                ch = (ch - 'A') + 'a';
        }
    }

    /**
     * @brief Return lower case copy of string.
     * 
     * @param[in] str string to convert.
     * @return std::string lower case string
     */
    [[nodiscard]] constexpr inline std::string toLower(std::string_view str)
    {
        std::string str_ = static_cast<std::string>(str);
        toLower(str_);
        return str_;
    }
